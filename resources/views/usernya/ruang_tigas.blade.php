...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Tiga</h1>
  <a href="/rtiga/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangtiga as $rtiga)
    <tr>
      <tr>
            <th scope="row">{{ $rtiga->id }}</th>
            <td>{{ $rtiga->nama_barang }}</td>
            <td>{{ $rtiga->spesifikasi }}</td>
            <td>{{ $rtiga->jumlah }}</td>
            <td>{{ $rtiga->kondisi }}</td>
    </tr>
   </tr>
