...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Lab ACP</h1>
  <a href="/rlacp/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruanglabacp as $rlacp)
    <tr>
      <tr>
            <th scope="row">{{ $rlacp->id }}</th>
            <td>{{ $rlacp->nama_barang }}</td>
            <td>{{ $rlacp->spesifikasi }}</td>
            <td>{{ $rlacp->jumlah }}</td>
            <td>{{ $rlacp->kondisi }}</td>
    </tr>
   </tr>
