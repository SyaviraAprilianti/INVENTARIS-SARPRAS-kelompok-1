...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Dua</h1>
  <a href="/rdua/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangdua as $dua)
    <tr>
      <tr>
            <th scope="row">{{ $dua->id }}</th>
            <td>{{ $dua->nama_barang }}</td>
            <td>{{ $dua->spesifikasi }}</td>
            <td>{{ $dua->jumlah }}</td>
            <td>{{ $dua->kondisi }}</td>
    </tr>
   </tr>
