...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Empat</h1>
  <a href="/rempat/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangempat as $rempat)
    <tr>
      <tr>
            <th scope="row">{{ $rempat->id }}</th>
            <td>{{ $rempat->nama_barang }}</td>
            <td>{{ $rempat->spesifikasi }}</td>
            <td>{{ $rempat->jumlah }}</td>
            <td>{{ $rempat->kondisi }}</td>
    </tr>
   </tr>
   @endforeach
  </tbody>
