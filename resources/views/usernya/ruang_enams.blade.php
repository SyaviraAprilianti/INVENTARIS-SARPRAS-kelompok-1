...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  ...
  <a href="/renam/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangenam as $renam)
    <tr>
      <tr>
            <th scope="row">{{ $renam->id }}</th>
            <td>{{ $renam->nama_barang }}</td>
            <td>{{ $renam->spesifikasi }}</td>
            <td>{{ $renam->jumlah }}</td>
            <td>{{ $renam->kondisi }}</td>
    </tr>
   </tr>
