...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Satu</h1>
  <a href="/rsatu/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangsatu as $rsatu)
    <tr>
      <tr>
            <th scope="row">{{ $rsatu->id }}</th>
            <td>{{ $rsatu->nama_barang }}</td>
            <td>{{ $rsatu->spesifikasi }}</td>
            <td>{{ $rsatu->jumlah }}</td>
            <td>{{ $rsatu->kondisi }}</td>
    </tr>
   </tr>
