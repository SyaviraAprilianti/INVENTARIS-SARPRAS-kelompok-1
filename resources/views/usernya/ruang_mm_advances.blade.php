...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang MM Advance</h1>
  <a href="/rmmadv/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangmmadvance as $rmmadv)
    <tr>
      <tr>
            <th scope="row">{{ $rmmadv->id }}</th>
            <td>{{ $rmmadv->nama_barang }}</td>
            <td>{{ $rmmadv->spesifikasi }}</td>
            <td>{{ $rmmadv->jumlah }}</td>
            <td>{{ $rmmadv->kondisi }}</td>
    </tr>
   </tr>
   @endforeach
  </tbody>
