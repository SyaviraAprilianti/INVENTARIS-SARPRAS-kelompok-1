...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang RPL Advance</h1>
  <a href="/rrpladv/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruangrpladvance as $rrpladv)
    <tr>
      <tr>
            <th scope="row">{{ $rrpladv->id }}</th>
            <td>{{ $rrpladv->nama_barang }}</td>
            <td>{{ $rrpladv->spesifikasi }}</td>
            <td>{{ $rrpladv->jumlah }}</td>
            <td>{{ $rrpladv->kondisi }}</td>
    </tr>
   </tr>
   @endforeach
  </tbody>
