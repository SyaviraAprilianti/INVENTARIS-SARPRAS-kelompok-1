...
<head>
    ...
    <title>Ruang Lab Broadcasting</title>
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Ruang Lab Broadcasting</h1>
  <a href="/rbc/pdf" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
</div>
...
<table class="table">
  ...
  <tbody>
  @foreach ($ruanglabbroadcasting as $rbc)
    <tr>
      <tr>
            <th scope="row">{{ $rbc->id }}</th>
            <td>{{ $rbc->nama_barang }}</td>
            <td>{{ $rbc->spesifikasi }}</td>
            <td>{{ $rbc->jumlah }}</td>
            <td>{{ $rbc->kondisi }}</td>
    </tr>
   </tr>
